...
class TransformerLM(torch.nn.Module):
    ...
    def forward(
        ...
        device = kwargs.get("device", args[1] if len(args) > 1 else torch.device("cuda"))
    ...
    # ):
    #     if ignore_eos:
    #         # For Qwen2LM, stop_token_ids contains all special end tokens.
    #         # We set their probabilities to negative infinity to prevent them from being sampled.
    #         if hasattr(self, 'stop_token_ids'):
    #             for token_id in self.stop_token_ids:
    #                 if token_id < len(weighted_scores): # Bounds check
    #                     weighted_scores[token_id] = -float('inf')
    #         else: 
    #             # Fallback for the base EOS token
    #             if self.speech_token_size < len(weighted_scores):
    #                 weighted_scores[self.speech_token_size] = -float('inf')

    #     # The sampling call is now safe and won't get stuck in a loop.
...
class Qwen2LM(TransformerLM):
    ...
    def forward(
        ...
        Args:
            ...
            audio_lengths: (B,)
        """
        # kwargs에서 필요한 인자를 직접 추출합니다.
        import sys
        print(f"--- [DEBUG] Qwen2LM forward called ---", file=sys.stderr)
        print(f"args: {type(args)}, len: {len(args)}", file=sys.stderr)
        print(f"kwargs: {kwargs.keys()}", file=sys.stderr)

        batch = kwargs.get("batch", args[0] if args else None)
        device = kwargs.get("device", args[1] if len(args) > 1 else torch.device("cuda"))

        if batch is None:
            print("--- [ERROR] batch is None! ---", file=sys.stderr)
            # kwargs에 혹시 batch와 유사한 데이터가 있는지 확인
            for key, value in kwargs.items():
                if isinstance(value, dict) and 'text_token' in value:
                    print(f"Found a batch-like dict in kwargs with key: '{key}'", file=sys.stderr)
                    batch = value
                    break
    ...
    def prepare_inputs_for_generation(self, input_ids, **kwargs):
        """
        PEFT 라이브러리와의 호환성을 위해 추가된 함수입니다.
        Hugging Face의 표준적인 생성 모델 인터페이스를 모방합니다.
        """

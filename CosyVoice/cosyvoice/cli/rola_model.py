...
class CosyVoiceModel:
    ...
    # CosyVoiceModel 클래스 내부
    def load(self, llm_model, flow_model, hift_model, lora_model_dir=None):
        # 1. 원본 LLM 가중치 로드
        # strict=False를 사용하여 LoRA 가중치가 없는 원본 모델을 유연하게 로드
        self.llm.load_state_dict(torch.load(llm_model, map_location=self.device), strict=False)
        print(f"Successfully loaded original LLM weights from {llm_model}")

        # 2. 파인튜닝된 LoRA 어댑터가 있다면 덮어쓰기
        if lora_model_dir:
            print(f"Loading LoRA adapter from {lora_model_dir}...")
            # PeftModel.from_pretrained를 사용하여 원본 모델 위에 어댑터를 로드
            self.llm = PeftModel.from_pretrained(self.llm, lora_model_dir)
            print("LoRA adapter loaded successfully.")

        self.llm.to(self.device).eval()

        # 3. 나머지 Flow, Hift 모델 로드
        self.flow.load_state_dict(torch.load(flow_model, map_location=self.device), strict=True)
        self.flow.to(self.device).eval()

        # hift_model이 hifigan 모델일 경우를 대비한 처리
        state_dict = torch.load(hift_model, map_location=self.device)
        # 체크포인트가 딕셔너리이고 'generator' 키를 포함하는지 확인
        if isinstance(state_dict, dict) and 'generator' in state_dict:
            state_dict = {k.replace('generator.', ''): v for k, v in state_dict['generator'].items()}
